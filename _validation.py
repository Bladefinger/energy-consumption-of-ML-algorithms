...
def cross_validate(estimator, X, y=None, groups=None, scoring=None, cv='warn',
    ...
    #energie_cost = _aggregate_score_dicts(energie_cost)

    ret = {}
    ret['fit_time'] = np.array(fit_times)
    ret['score_time'] = np.array(score_times)
    #ret['energie'] = np.array(energie_cost)
...
def cross_val_score(estimator, X, y=None, groups=None, scoring=None, cv='warn',
    ...
    cv_results = cross_validate(estimator=estimator, X=X, y=y, groups=groups,
                                ...
                                pre_dispatch=pre_dispatch,
                                error_score=error_score)
    return cv_results['test_score'],cv_results['energie']


# Cette fonction contient le code exécuté par la fonction fit_and_score
pandas_handler = PandasHandler()
@measure_energy(handler=pandas_handler)
def _fit_and_score_compute(estimator, X, y, scorer, train, test, verbose,
...
def _fit_and_score(estimator, X, y, scorer, train, test, verbose,
    ...
    with EnergyContext(handler=pandas_handler, domains=[RaplPackageDomain(0)], start_tag='model') as ctx:
    
        meter.start(tag='model')
        ...
        try:
            ...
            if error_score == 'raise':
                ...
                warnings.warn("From version 0.22, errors during fit will result "
                            ...
                            FutureWarning)
                raise
            elif isinstance(error_score, numbers.Nsumber):
                if is_multimetric:
    ...
    ret.append({'energie': df.package_0[0]})
